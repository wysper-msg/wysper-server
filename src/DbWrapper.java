...
class DbWrapper {
  ...
  void insertUser(String username) {
    ...
    try {
      psInsert = conn.prepareStatement("insert into users (username, last_read) values (?, ?)");
  ...
  void insertMessage(Message msg) {
    ...
    try {
      psInsert =
          conn.prepareStatement("insert into messages (username, time, text) values (?, ?, ?)");
      this.statements.add(psInsert);
  ...
  ArrayList<Message> getMessages(String username) {
    ...
    try {
      ...
      getMessage =
          conn.prepareStatement("SELECT username, text, time, mid  from messages WHERE mid > ?");
      ...
      // Finally we should update the user to reflect that they have read up to the most recent
      // message
  ...
  ArrayList<Message> getMessages(int n) {
    ...
    try {
      getmessage =
          conn.prepareStatement(
              "SELECT username, mid, text, time  from messages ORDER BY mid DESC");
      rs = getmessage.executeQuery();
      if (!rs.next()) {
        return ret;
      } else {
  ...
  void displayAllUsers() {
    ...
    try {
      ...
      if (!rs.next()) {
        ...
        do {
          // fetching messages(Add code here to enter values to message class object)
          System.out.println(String.format("%10s %10s", rs.getString(1), rs.getInt(2)));
        } while (rs.next());
      }
      rs.close();
    } catch (SQLException sqle) {
